...
header("Access-Control-Allow-Origin: *");
    ...
    private function miniCal($user)
    {
        ...
        return $miniCal->getCalendarHtml(array('showToday' => TRUE,
    ...
    private function geniCal($calendars, $mode = 'ical')
    {
		...
		$calContent.= "REFRESH-INTERVAL;VALUE=DURATION:PT12H\n";
		$calContent.= "X-PUBLISHED-TTL:PT12H\n";
                    ...
                    foreach ($dates as $date => $entry) {
                        ...
                        */
                        ...
                        if ($mode == 'google') {
                            ...
                            $calContent.= "DTSTAMP:".date('Ymd', strtotime($date))."T".date('His', strtotime($date)). "\n";
    ...
    private function getColor($string)
    {
        ...
        $c = array("R" => hexdec(substr($checksum, 0, 2)),
    ...
    private function clean_url()
    {
        ...
        $raw = array_merge(array('controller' => 'cal',
            'action' => 'index',
            'd' => $this->_request->getParam('d'),
            ...
            'cd' => $this->_request->getParam('cd')), $_GET);
    ...
    if ($m == 'google'){
        $parts = array('controller' => 'cal',
            ...
            'g' => $this->calg, // grade
            's' => $this->cals, // subject(s)
            'm' => $m); // mode (ical/google)
        $url = $this->view->url($parts, null, true, true);
        return "https://www.google.com/calendar/render?cid=webcal://" . $_SERVER['SERVER_NAME'] . $url;
    }elseif ($m == 'outlook'){
        $parts = array('controller' => 'cal',
            'action' => 'index',
            'd' => $this->cald, // date
